...
test("操作提示条按模式切换；指针锁定生命周期；无鱼叉点击给购置指引", async ({ page }) => {
  await boot(page);

  // 航行模式：提示"点击锁定视角" + "无武器·去船坞"警示（尚未购置鱼叉炮）
  const hint = page.locator("#controls-hint");
  await expect(hint).toContainText(/Click screen to lock|点击画面锁定/);
  await expect(hint.locator(".hint-warn")).toBeVisible();
  await expect(hint).toContainText(/shipwright|船坞/);

  // headless Chromium 不支持指针锁定（WrongDocumentError），这里验证的是降级路径：
  // 首次点击尝试锁定（该次点击不算攻击）→ 失败标记不可用 → 后续点击恢复攻击语义。
  // 真实浏览器的锁定/Esc 释放走标准 Pointer Lock API，无法在 headless 端到端验证。
  await page.mouse.click(640, 400);
  await page.waitForTimeout(400); // 等锁定请求 reject 落地（lockUnavailable 置位）
  await page.mouse.click(640, 400);
  await expect(page.locator(".toast")).toHaveClass(/visible/);
  await expect(page.locator(".toast")).toContainText(/Harpoon Cannon|鱼叉炮/);

  // 上岸后：提示条切换为步行操作（跳跃/攻击），无武器警示消失
  await page.evaluate(() => window.__game!.goAshore());
  await expect(hint).toContainText(/Jump|跳跃/);
  await expect(hint.locator(".hint-warn")).toHaveCount(0);

  // 购置鱼叉炮并回船：警示位变为"发射鱼叉"
  await page.evaluate(() => {
    const g = window.__game!;
    g.setState({ ...(g.getState() as object), harpoon: true });
    g.boardShip();
  });
  await expect(hint).toContainText(/Fire harpoon|发射鱼叉/);
});

test("切换语言后聊天记录整体重译，玩家发言保持原文", async ({ page }) => {
  // 以中文启动：系统欢迎语为中文
  await page.goto("/");
  await page.evaluate(() => localStorage.setItem("sandsea-lang", "zh"));
  await page.reload();
  await page.waitForFunction(() => Boolean(window.__game));
  await page.evaluate(() => window.__game!.clearSave());
  const log = page.locator("#chat-log");
  await expect(log).toContainText("欢迎来到沙海");

  // 玩家发一条原文消息
  await page.keyboard.press("Enter");
  await page.locator("#chat-input").fill("hello sandsea");
  await page.keyboard.press("Enter");
  await expect(log).toContainText("hello sandsea");

  // 切到英文：历史系统消息重译为英文，玩家原文不动
  await page.locator("#lang-toggle").click();
  await expect(log).toContainText("Welcome to the Sandsea");
  await expect(log).not.toContainText("欢迎来到沙海");
  await expect(log).toContainText("hello sandsea");
  await expect(log).toContainText("Captain (you)");

  // 切回中文：再次整体重译
  await page.locator("#lang-toggle").click();
  await expect(log).toContainText("欢迎来到沙海");
  await expect(log).toContainText("船长（你）");
  await expect(log).toContainText("hello sandsea");
});
