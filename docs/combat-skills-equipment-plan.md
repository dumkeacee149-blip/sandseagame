...
- 任务链：每完成 2 环 +1 点（floor(claimedQuests/2)，14 环 → 7 点）
- 里程碑：访满三港 +1、通关 +2、累计卖货 500g +1（tradeGold 字段）
- 满配约 11 点 < 15 个技能位：三分支必须取舍，**不可洗点**（原型期简化；
  洗点是后续付费/代币出口的候选，先留白）

### 4.2 三分支（每层 1 点，线性解锁）

```
航海：帆效+4% → 搁浅掉货-10pt → 咬击掉货-5pt → 帆效+6% → 拖船费全免
战斗：近战+3 → 鱼叉+4 → 冷却-10% → 近战+5 → 鱼叉暴击10%×2倍
商贾：卖价+2% → 货舱+2 → 买价-2% → 卖价+3% → 货舱+4
```
...
3. **playerHp 数值**：60、沙蟹 8/击、脱战 5s 后 8/s 缓回——已按此实现；
   HP 为运行时状态不入存档（回血机制下持久化无意义，也避免回血 tick 刷写
   localStorage），死亡惩罚 = 拖回最后交易港重生、材料不掉。
